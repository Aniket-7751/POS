...
const AddCategoryPage: React.FC<AddCategoryPageProps> = ({ onBack, editId, editData }) => {
  ...
  useEffect(() => {
    if (!editData) {
      fetch('http://localhost:5050/api/categories/generate-id')
        .then(res => res.json())
        .then(data => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.categoryName || !form.organizationId) {
      setError('Please fill all required fields (Category Name, Organization)');
      return;
    }
